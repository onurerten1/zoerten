*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZOE_V003
*   generation date: 10.04.2020 at 15:41:40
*   view maintenance generator version: #001407#
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZOE_V003            .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZOE_V003'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZOE_V003_EXTRACT
TOTAL = ZOE_V003_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZOE_V003'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZOE_V003-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZOE_V003_EXTRACT
TOTAL = ZOE_V003_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZOE_V003-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZOE_V003'
...
TOTAL = ZOE_V003_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZOE_V003-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZOE_V003-UPD_FLAG NE SPACE.
